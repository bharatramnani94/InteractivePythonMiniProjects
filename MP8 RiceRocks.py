# RiceRocks (Final)
# http://www.codeskulptor.org/#user38_lp2OnARwZZ_7.py
...
class Sprite:
    ...
    def collide(self, other_object):
        a1 = self.pos[0]
        a2 = self.pos[1]
        b = other_object.get_position()
        b1 = b[0]
        b2 = b[1]
        r1 = self.radius
        r2 = other_object.get_radius()
        
        # determine distance d between the two
        d = math.sqrt((b2-a2)**2 + (b1-a1)**2)
...
def draw(canvas):
    ...
    if lives < 1:
        started = False
        lives = 0
        for g in set(rock_group):
            rock_group.discard(g)
            soundtrack.rewind()
...
def rock_spawner():
    if started:
        ...
        a = a_rock.get_position()
        b = my_ship.get_position()
        a1 = a[0]
        a2 = a[1]
        b1 = b[0]
        b2 = b[1]
        ...
        if (len(rock_group) < 12):
            if (math.sqrt((b2-a2)**2 + (b1-a1)**2) > 100):
                rock_group.add(a_rock)

# take a set and a canvas and call the update and draw methods for each sprite in the group
def process_sprite_group(set_s,canvas):
    for s in set(set_s):
        s.draw(canvas)
        s.update()
        if s.update():	# age exceeds lifespan
            set_s.remove(s)
